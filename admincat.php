...
<nav class = "flex px-5 py-3 text-gray-700 justify-between rounded-lg bg-gray-50 dark:bg-[#1E293B] " aria-label="Breadcrumb">
            <ol class = "inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li class = "inline-flex items-center">
                    <a href="addcat.php" class = " px-4 py-2 rounded bg-gray-800 hover:bg-gray-900 inline-flex items-center text-sm font-medium text-white hover:text-white dark:text-white dark:hover:text-white">
            ...
            <div class="w-96 md:w-80">
                <form action="admincat.php" method="get" class="w-full flex relative rounded-full border-1 overflow-hidden   border border-gray-800/10">
    ...
    <div class="overflow-x-scroll">
                            <table class="w-full whitespace-nowrap">
                                ...
                                <?php 
                                 $list=$connect->query("SELECT * FROM categorie ORDER BY cat_id desc");
                                 if (isset($_GET['submit']) && !empty($_GET['search'])) {
                                      $all= htmlspecialchars($_GET['search']);
                                      $list=$connect->query("SELECT * FROM categorie WHERE nom LIKE '%".$all."%' ORDER BY cat_id desc");
                                 }
                                 if ($list->rowCount()>0) {
                                   ...
                                   <td class="py-3 px-2"><?php echo $a['cat_id'] ;?></td>
                                    ...
                                    <td class="py-3 px-2">
                                        <div class="inline-flex items-center space-x-3">
                                            <a href="cat_edit.php?id=<?php echo $a['cat_id'];?>" title="Edit" class="hover:text-white"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                            ...
                                            <a href="" title="Edit password" class="hover:text-white"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                              ...
                                              <form action="cat_code.php" method="post">
                                                <button type="submit" name="delete_cat"  value="<?php echo $a['cat_id'];?>" class="mt-2">
                                    ...
                                    <h1 class="text-2xl text-red-600 font-serif font-medium text-center">Aucune categorie trouvée</h1>
                                <?php  
                                }
                                ;?>
