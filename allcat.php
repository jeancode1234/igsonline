...
<body class="bg-indigo-100">
    ...
    <div class="w-full h-auto py-14 flex items-center justify-center bg-pink-500">
        <div class="w-full md:max-w-4xl space-y-8">
            <h1 class="text-4xl lg:text-6xl font-bold text-center text-gray-800">Trouvez la catégorie de votre choix en un clic</h1>
            <div class="w-full md:max-w-2xl lg:max-w-xl mx-auto">
                <form action="allcat.php" method="get" class="w-full flex justify-center relative rounded-full overflow-hidden">
                    <input type="text" autocomplete="off" name="search" placeholder="Entrez votre recherche ici..." class="w-5/6  px-2 py-4  bg-white  focus:outline-none border-none">
    ...
    <div class="w-full container sm:max-w-full  mx-auto px-4 space-y-8 py-4">
        <div class="grid grid-cols-2 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-12 grid-flow-row gap-4">
            ...
            <?php $req = $connect->query("SELECT * FROM categorie ORDER BY cat_id desc");
            if (isset($_GET['submit']) && !empty($_GET['search'])) {
                $all= htmlspecialchars($_GET['search']);
                $req=$connect->query("SELECT * FROM categorie WHERE nom LIKE '%".$all."%' ORDER BY cat_id desc");
            }
            if ($req->rowCount() > 0) {
